...
%% 
% Time VS Middle Node Velocity
figure (2);
timeArray =  (1:N_steps)*dt;
plot(timeArray, all_mid_yv, 'k-');
xlabel('Time, t [sec]');
ylabel('Velocity of mid-node, v [meter/sec]')
title('Middle Node Velocity VS Time');
grid on;


% Time VS Maximum Displacement
figure (3);
plot(timeArray, y_max, 'k-');
xlabel('Time, t [sec]');
ylabel('Maximum Displacement, p [meter]');
title('Maximum Displacement VS Time');
grid on;

%% Euler beam theory Calculation
c = min(d, l-d);
Euler_beam_theory = (P*c*(l^2 - c^2)^1.5)/(9*sqrt(3)*EI*l);
fprintf('The Maximum Beam Displacement from Euler beam theory is \n%1.4f', Euler_beam_theory);
